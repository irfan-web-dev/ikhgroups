import React from "react";

const clientLogos = [
  {
    url: "https://images.seeklogo.com/logo-png/10/1/petron-logo-png_seeklogo-108079.png",
    name: "Petron",
  },
  {
    url: "https://www.england.nhs.uk/nhsidentity/wp-content/uploads/sites/38/2016/04/nhs-logo-digital-space.jpg",
    name: "NHS",
  },
  {
    url: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTNWFf1HA9pjjt8UX86nQxrFgOiIAXu5zchQA&s",
    name: "Daelim",
  },
  {
    url: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTOhxT88xdW2L6aluuOTj3aNAajTFN4eBNs0A&s",
    name: "HEC",
  },
  {
    url: "https://cdn.brandfetch.io/idRRbNH8sS/theme/dark/logo.svg?c=1dxbfHSJFAPEGdCLU4o5B",
    name: "Arcelor Mittal",
  },
  {
    url: "https://www.logo.wine/a/logo/Abengoa/Abengoa-Logo.wine.svg",
    name: "Abengoa",
  },
  {
    url: "https://images.seeklogo.com/logo-png/36/1/yasref-logo-png_seeklogo-369851.png",
    name: "YASREF",
  },
  {
    url: "https://www.saudifal.com.sa/img/logo2-1.png",
    name: "Saudi Fal",
  },
  {
    url: "https://logowik.com/content/uploads/images/samsung-engineering3687.logowik.com.webp",
    name: "Samsung Engineering",
  },
  {
    url: "https://upload.wikimedia.org/wikipedia/commons/thumb/4/48/Samsung_C%26T_logo.svg/2560px-Samsung_C%26T_logo.svg.png",
    name: "Samsung C&T",
  },
  {
    url: "https://logowik.com/content/uploads/images/saipem5622.logowik.com.webp",
    name: "Saipem",
  },
  {
    url: "https://images.seeklogo.com/logo-png/6/1/hyundai-engineering-logo-png_seeklogo-69059.png",
    name: "Hyundai Engineering",
  },
  {
    url: "https://images.seeklogo.com/logo-png/6/1/hyundai-heavy-industries-logo-png_seeklogo-69063.png",
    name: "Hyundai Heavy Industries",
  },
  {
    url: "https://hloljob.com//images/alhajricorporation.png",
    name: "Nasser S. Al Hajri Corporation",
  },
  {
    url: "https://images.seeklogo.com/logo-png/12/1/snc-lavalin-logo-png_seeklogo-128474.png",
    name: "SNC Lavalin",
  },
  {
    url: "https://logowik.com/content/uploads/images/technip4442.logowik.com.webp",
    name: "Technip",
  },
  {
    url: "https://www.sudelettra.com/wp-content/uploads/2017/02/logo-sudelettra.png",
    name: "Sudelettra",
  },
  {
    url: "https://images.seeklogo.com/logo-png/47/1/sraco-logo-png_seeklogo-472850.png",
    name: "SRACO",
  },
  {
    url: "https://companieslogo.com/img/orig/2310.SR_BIG-7ac54b87.png?t=1720244490",
    name: "Sipchem",
  },
  {
    url: "https://images.seeklogo.com/logo-png/28/1/sinopec-logo-png_seeklogo-284996.png",
    name: "Sinopec",
  },
  {
    url: "data:image/png;base64,",
    name: "Kingdom of Saudi Arabia",
  },
  {
    url: "https://logowik.com/content/uploads/images/689_holidayinn.jpg",
    name: "Holiday Inn",
  },
  {
    url: "https://images.seeklogo.com/logo-png/21/1/sadara-chemical-company-logo-png_seeklogo-213687.png",
    name: "Sadara",
  },
  {
    url: "https://images.seeklogo.com/logo-png/12/1/sabic-logo-png_seeklogo-121234.png",
    name: "SABIC",
  },
  {
    url: "https://images.seeklogo.com/logo-png/31/1/petrofac-logo-png_seeklogo-310770.png",
    name: "Petrofac",
  },
  {
    url: "https://images.seeklogo.com/logo-png/12/1/saudi-electricity-company-logo-png_seeklogo-122931.png",
    name: "Saudi Electricity Company",
  },
  {
    url: "https://images.seeklogo.com/logo-png/30/1/saudi-aramco-logo-png_seeklogo-306746.png",
    name: "Saudi Aramco",
  },
  {
    url: "https://images.seeklogo.com/logo-png/2/1/carlo-gavazzi-logo-png_seeklogo-26353.png",
    name: "Carlo Gavazzi",
  },
  {
    url: "https://images.seeklogo.com/logo-png/34/1/chiyoda-corporation-logo-png_seeklogo-341759.png",
    name: "Chiyoda Corporation",
  },
  {
    url: "https://www.chinachamber.org.uk/wp-content/uploads/2025/01/BCEGI_Logo-01-2048x646.jpg",
    name: "BCEGI",
  },
  {
    url: "https://images.seeklogo.com/logo-png/38/1/bonatti-logo-png_seeklogo-386691.png",
    name: "Bonatti",
  },
  {
    url: "https://www.rtcc.com.sa/wp-content/uploads/2023/01/logo-03-e1672981302803.png.webp",
    name: "Al Rashid trading & Contracting Co",
  },
  {
    url: "https://www.arccksa.com/img/logos/ARCC-logo.png",
    name: "Al Rushaid Construction Co. Ltd.",
  },
  // {
  //   url: "https://argaamplus.s3.amazonaws.com/911df464-fbb4-4909-8f1d-767f088a5302.png",
  //   name: "Azmeel",
  // },
  {
    url: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTix1fsoEi70g-RbCC8h41LXpBF6aVRjo75EHz0oRamAphHWbIZiKEicZb4cCPXBX4_NFY&usqp=CAU",
    name: "Al Mashaqirq Group",
  },
  {
    url: "https://www.albiladarabia.com/wp-content/uploads/2023/09/logo.png",
    name: "Al Bilad Arabia",
  },
  {
    url: "https://images.seeklogo.com/logo-png/21/1/larsen-toubro-lt-logo-png_seeklogo-211460.png",
    name: "Larsen & Toubro",
  },
  {
    url: "https://images.seeklogo.com/logo-png/31/1/maaden-logo-png_seeklogo-316199.png",
    name: "Ma'aden",
  },
  {
    url: "https://images.seeklogo.com/logo-png/54/1/satorp-logo-png_seeklogo-544812.png",
    name: "SATORP",
  },
  {
    url: "https://images.seeklogo.com/logo-png/4/1/esb-logo-png_seeklogo-48822.png",
    name: "ESB",
  },
  {
    url: "https://images.seeklogo.com/logo-png/0/1/applus-logo-png_seeklogo-9881.png",
    name: "Applus",
  },
  {
    url: "https://media.licdn.com/dms/image/v2/C4E0BAQFLGxUFfdwGHw/company-logo_200_200/company-logo_200_200/0/1630604527863?e=2147483647&v=beta&t=HOCr37yjNWKPKbAshdo_cybqtGuxhx18xePCgJMtUzY",
    name: "GESAT",
  },
  {
    url: "https://images.seeklogo.com/logo-png/30/1/alfanar-logo-png_seeklogo-304076.png",
    name: "Alfanar",
  },
];

export default function ClientsSection() {
  const trackRef = React.useRef(null);
  const [isAnimating, setIsAnimating] = React.useState(false);
  const itemsToMove = 3;
  const animationTimeout = React.useRef(null);

  // Clean up timeout on unmount
  React.useEffect(() => {
    return () => {
      if (animationTimeout.current) {
        clearTimeout(animationTimeout.current);
      }
    };
  }, []);

  const handleNext = () => {
    if (isAnimating || !trackRef.current) return;

    setIsAnimating(true);
    const track = trackRef.current;
    const items = Array.from(track.children);
    if (items.length === 0) return;

    const itemWidth = items[0].offsetWidth;
    const moveDistance = itemWidth * itemsToMove;

    track.style.transition = "transform 0.5s ease";
    track.style.transform = `translateX(-${moveDistance}px)`;

    // Fallback timeout in case transitionend doesn't fire
    animationTimeout.current = setTimeout(() => {
      completeNextAnimation();
    }, 200); // Slightly longer than the transition duration

    const completeNextAnimation = () => {
      if (animationTimeout.current) {
        clearTimeout(animationTimeout.current);
        animationTimeout.current = null;
      }

      // Move the first 'itemsToMove' items to the end
      for (let i = 0; i < itemsToMove; i++) {
        if (items.length > 0) {
          track.appendChild(items[0]);
        }
      }

      track.style.transition = "none";
      track.style.transform = "translateX(0)";
      setIsAnimating(false);
    };

    track.addEventListener("transitionend", completeNextAnimation, {
      once: true,
    });
  };

  const handlePrev = () => {
    if (isAnimating || !trackRef.current) return;

    setIsAnimating(true);
    const track = trackRef.current;
    const items = Array.from(track.children);
    if (items.length === 0) return;

    const itemWidth = items[0].offsetWidth;
    const moveDistance = itemWidth * itemsToMove;

    // Move the last 'itemsToMove' items to the beginning
    for (let i = 0; i < itemsToMove; i++) {
      if (items.length > 0) {
        track.insertBefore(items[items.length - 1], items[0]);
      }
    }

    track.style.transition = "none";
    track.style.transform = `translateX(-${moveDistance}px)`;

    requestAnimationFrame(() => {
      track.style.transition = "transform 0.5s ease";
      track.style.transform = "translateX(0)";

      // Fallback timeout
      animationTimeout.current = setTimeout(() => {
        completePrevAnimation();
      }, 200);

      const completePrevAnimation = () => {
        if (animationTimeout.current) {
          clearTimeout(animationTimeout.current);
          animationTimeout.current = null;
        }
        setIsAnimating(false);
      };

      track.addEventListener("transitionend", completePrevAnimation, {
        once: true,
      });
    });
  };

  return (
    <section className="section clients-section visible" id="client-component">
      <div className="container">
        <div className="section-title">
          <h2>Our Clients</h2>
          <p className="section-description">
            Trusted by leading companies across industries
          </p>
        </div>

        <div className="client-slider">
          <button
            className="client-nav-button prev"
            onClick={handlePrev}
            disabled={isAnimating}
            aria-label="Previous"
          >
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M15 18L9 12L15 6"
                stroke="currentColor"
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
              />
            </svg>
          </button>

          <div className="client-track" ref={trackRef}>
            {clientLogos.map((client, idx) => (
              <div
                key={client.url + idx}
                className="client-item"
                style={{
                  display: "flex",
                  flexDirection: "column",
                  alignItems: "center",
                }}
              >
                <img
                  src={client.url}
                  className="client-logo"
                  alt={client.name}
                />
                <div className="client-name">{client.name}</div>
              </div>
            ))}
          </div>

          <button
            className="client-nav-button next"
            onClick={handleNext}
            disabled={isAnimating}
            aria-label="Next"
          >
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M9 18L15 12L9 6"
                stroke="currentColor"
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
              />
            </svg>
          </button>
        </div>
      </div>

      <style jsx>{`
        .client-nav-button {
          position: absolute;
          top: 50%;
          transform: translateY(-50%);
          background: rgba(255, 255, 255, 0.8);
          border: 1px solid #ddd;
          border-radius: 50%;
          width: 40px;
          height: 40px;
          display: flex;
          align-items: center;
          justify-content: center;
          cursor: pointer;
          z-index: 10;
          transition: all 0.3s ease;
          color: #333;
        }

        .client-nav-button:disabled {
          opacity: 0.5;
          cursor: not-allowed;
        }

        .client-nav-button:hover:not(:disabled) {
          background: #fff;
          box-shadow: 0 2px 5px rgba(0, 0, 0, 0.1);
          color: #000;
        }

        .prev {
          left: 0;
        }

        .next {
          right: 0;
        }

        .client-nav-button svg {
          width: 20px;
          height: 20px;
        }
      `}</style>
    </section>
  );
}
